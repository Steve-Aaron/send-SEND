...
export function ConsultationProcess() {
  ...
  const stages = [
    ...
    },
    {
      ...
      fullDesc:
        "After the consultation closes, all submissions are analyzed. The government then publishes a response detailing which proposals will move forward into law (often as a White Paper or direct legislation). Strong public pushback during stage II is the only way to prevent harmful policies from reaching this final stage.",
  ...
  return (
    <section className="bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-border-subtle relative overflow-hidden">
      ...
      <h3 className="text-3xl font-serif font-bold text-brand mb-12 relative z-10">
        ...
        {stages.map((stage) => (
          <div key={stage.id} className="relative pl-16 group">
            ...
            <div className="bg-bg-subtle rounded-2xl p-6 border border-border-subtle transition-all duration-300 hover:shadow-md hover:border-accent/30 relative">
              <h4 className="text-xl font-serif font-bold text-brand mb-2 pr-10">
                {stage.title}
              </h4>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 pb-20 sm:pb-6">
          <div
            className="absolute inset-0 bg-brand/80 backdrop-blur-sm transition-opacity"
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden relative z-10 transform transition-all animate-fade-in flex flex-col max-h-[90vh]">
            ...
            <div className="relative h-48 sm:h-64 shrink-0">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <div className="text-accent font-bold uppercase tracking-widest text-sm mb-1">
                  Stage {activeModal.id}
                </div>
